...
public final class JsonSchemaConverter implements ToSmithyBuilder<JsonSchemaConverter> {
    ...
    private final ShapeIndex originalShapeIndex;
    ...
    private JsonSchemaConverter(Builder builder) {
        ...
        originalShapeIndex = SmithyBuilder.requiredState("shapeIndex", builder.shapeIndex);
        ...
        if (builder.rootShape == null) {
            ...
            rootShape = builder.shapeIndex.getShape(builder.rootShape)
                    .orElseThrow(() -> new SmithyJsonSchemaException(
                            "Invalid root shape (shape not found): " + builder.rootShape));
        }

        shapeIndex = createIndex(originalShapeIndex, rootShape, builder.shapePredicate, builder.config);

        LOGGER.fine("Creating JSON ref strategy");
        refStrategy = RefStrategy.createDefaultStrategy(shapeIndex, config, propertyNamingStrategy);
        ...
        visitor = new JsonSchemaShapeVisitor(shapeIndex, this, realizedMappers);
    ...
    private static ShapeIndex createIndex(
            ShapeIndex index,
            Shape rootShape,
        ...
        if (rootShape != null) {
            ...
            Set<Shape> connected = new Walker(model.getShapeIndex()).walkShapes(rootShape);
        ...
        boolean includePrivate = config.getBooleanMemberOrDefault(JsonSchemaConstants.INCLUDE_PRIVATE_SHAPES);
        if (!includePrivate) {
            LOGGER.fine("Removing shapes marked as private");
            model = transformer.removeShapesIf(model, shape -> shape.hasTrait(PrivateTrait.class));
        }
    ...
    public String toPropertyName(MemberShape member) {
        Shape containingShape = shapeIndex.getShape(member.getContainer())
    ...
    public SchemaDocument convert() {
        ...
        Predicate<Shape> rootShapePredicate = shape -> true;
        if (rootShape != null) {
            Set<Shape> connected = new Walker(shapeIndex).walkShapes(rootShape);
            rootShapePredicate = connected::contains;
        }
